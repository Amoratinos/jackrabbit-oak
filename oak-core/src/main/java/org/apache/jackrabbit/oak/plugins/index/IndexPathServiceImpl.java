    ...
    public Iterable<String> getIndexPaths() {
        ...
        if (!indxDefnTypeIndexed) {
            ...
            return CollectionUtils.toStream(oakIndex.getChildNodeEntries())
                    ...
                    .map(cne -> PathUtils.concat("/oak:index", cne.getName())).collect(Collectors.toList());
        } else {
            Iterator<IndexRow> itr = getIndex().query(createFilter(INDEX_DEFINITIONS_NODE_TYPE), nodeStore.getRoot());
            return CollectionUtils.toStream(itr).map(input -> input.getPath()).collect(Collectors.toList());
        }
    }
